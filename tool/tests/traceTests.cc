...
TEST_F(TraceTest, BasicTypesVCD) {
  VCDTraceReaderConfig config;
  config._clk = "clk";
  config._selectedScope = "tb::DUT";
  tr = new VCDtraceReader("../tests/input/BasicTypes.vcd", config);
  const TracePtr &trace = tr->readTrace();
...
TEST_F(TraceTest, MergedVCD) {
  ...
  config._clk = "clk";
  config._selectedScope = "tb::DUT";
  tr = new VCDtraceReader(traces, config);
  const TracePtr &trace = tr->readTrace();
  BooleanVariablePtr o1 = trace->getBooleanVariable("o1");
...
TEST_F(TraceTest, SignedLogic) {
  ...
  VCDTraceReaderConfig config;
  config._clk = "clk";
  config._selectedScope = "SignedTypes::dut";
  tr = new VCDtraceReader("../tests/input/SignedTypes.vcd", config);
  const TracePtr &trace = tr->readTrace();
