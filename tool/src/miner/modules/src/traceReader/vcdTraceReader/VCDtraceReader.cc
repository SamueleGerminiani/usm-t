...
TracePtr VCDtraceReader::readTrace(const std::string file) {
    ...
    if (n_s.second.size() > 1) {
      ...
      for (auto &s : n_s.second) {
        if (indexes.count(s->lindex)) {
          messageWarning(
              "Splitted signal '" + n_s.first +
              "' has multiple bits with the same index " +
                  std::to_string(s->lindex) + ", it will be ignored\n");
        }
      ...
      //remove duplicates
      n_s.second.erase(
          std::unique(begin(n_s.second), end(n_s.second),
                      [](VCDSignal *e1, VCDSignal *e2) {
                        return e1->lindex == e2->lindex;
                      }),
          end(n_s.second));
  ...
  delete vcd_trace;
  //remove the clock signal from the trace
  trace->removeVariableByName(_config._clk);
