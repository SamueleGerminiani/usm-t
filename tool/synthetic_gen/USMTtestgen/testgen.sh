...
# Usage: ./generate_tests.sh <output_folder> <config_folder> <test_name> <clock_name> <reset_signal> <vcd_scope>

if [ "$#" -ne 6 ]; then
    echo "Usage: $0 <output_folder> <test_name> <clock_name> <reset_signal> <vcd_scope> <tracelenght>"
    exit 1
fi
...
GENERATE_OUTPUT=$(python3 ./src/generate_from_ltl.py "--parallel" "1" "--clk" "$CLOCK_NAME" "--outdir" "$OUTPUT_FOLDER" "--templates" "\"{G(..&&.. |=> F ..&&..),3,2,2,3}\"" "--tracelenght" "$TRACELENGHT")
...
./testGeneration/generateTest.sh "$OUTPUT_FOLDER" "$CONFIG_FOLDER" "$TEST_NAME" "$TOP_MODULE" "$CLOCK_NAME" "$RESET_SIGNAL" "$VCD_SCOPE" "$RETURNED_STRING"
