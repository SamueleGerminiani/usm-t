...
param_list="1,1,10,0 2,1,10,0 2,2,10,0 3,2,10,0 3,3,10,0 4,3,10"
...
if [ ! -f "$USMT_ROOT/tool/synthetic_gen/raw_challenges/Next_Results/summary_report.csv" ]; then
    echo "Experiment,Nant,Ncon,Nspec,Overlap,Elapsed_Time(s),Design_Lines" > "$USMT_ROOT/tool/synthetic_gen/raw_challenges/Next_Results/summary_report.csv"
fi
...
for param in $param_list; do
    ...
    out_dir="$next_results_dir/Next_A${nant}_C${ncon}_S${nspec}_O${overlap}"
    ...
    mv "$out_dir/.elapsed.csv" "$elapsed_dir/Next_A${nant}_C${ncon}_S${nspec}_O${overlap}_elapsed.csv"

    bash "$USMT_ROOT/tool/synthetic_gen/testGenerator/input_generator/count_design_lines.sh" "$out_dir" > "$other_stats_dir/Next_A${nant}_C${ncon}_S${nspec}_O${overlap}_design_size.txt"

    # extract elapsed time from the .elapsed.csv file, second line after the comma
    elapsed_time=$(sed -n '2p' "$elapsed_dir/Next_A${nant}_C${ncon}_S${nspec}_O${overlap}_elapsed.csv" | cut -d',' -f2)
    design_lines=$(cat "$other_stats_dir/Next_A${nant}_C${ncon}_S${nspec}_O${overlap}_design_size.txt")
    # append results to summary_report.csv
    echo "Next,${nant},${ncon},${nspec},${overlap},${elapsed_time},${design_lines}" >> "$USMT_ROOT/tool/synthetic_gen/raw_challenges/Next_Results/summary_report.csv"
