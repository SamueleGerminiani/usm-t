...
def generateCSV():
    ...
    try:
        subprocess.run(f"{globals.root}/tool/build/vcd2csv --vcd-dir {globals.out_folder}faulty_traces/vcd --clk {globals.clk_name} --vcd-ss \"top_module_bench::top_module_\" --dump-to {globals.out_folder}faulty_traces/csv",stdout=subprocess.DEVNULL if not globals.debug else None,stderr=subprocess.DEVNULL if not globals.debug else None, shell=True, check=True)
    except subprocess.CalledProcessError as e:
