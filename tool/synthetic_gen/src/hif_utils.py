...
def generate_injectable_design():
    ...
    verilog_files = subprocess.run(f"find {globals.hif_tb_prefix}rtl/ -name '*.v'", shell=True, check=False, capture_output=True, text=True).stdout.strip().replace('\n', ' ')
...
def run_hifsuite(specification):
    ...
    try:
        subprocess.run(f"cp {globals.out_folder}*.v {globals.hif_tb_prefix}/rtl", stdout=subprocess.DEVNULL, shell=True, check=True)
    except subprocess.CalledProcessError as e:
        print(globals.CERR +"Error:" + globals.CEND + f"Failed to copy files to {globals.hif_tb_prefix}/rtl. {e}")
        exit(1)    
