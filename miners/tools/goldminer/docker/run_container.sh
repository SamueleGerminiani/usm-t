input_mount_volume=$1
output_mount_volume=$2
com=$3
docker run -e MINED_SPECIFICATIONS_FILE=$MINED_SPECIFICATIONS_FILE -u root --rm -it -v $input_mount_volume:/input:rw -v $output_mount_volume:/output:rw samger/goldminer2:latest $com

input_mount_volume=$1
output_mount_volume=$2
command_to_run_the_miner=$3
xml_exported_vars=$4

docker run \
--cpus $MAX_N_CORES \
-e MINED_SPECIFICATIONS_FILE=$MINED_SPECIFICATIONS_FILE $xml_exported_vars \
-u root --rm -it \
-v $input_mount_volume:/input:rw \
-v $output_mount_volume:/output:rw \
samger/goldminer2:latest \
$command_to_run_the_miner

